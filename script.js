let filter_btnBrand = document.getElementsByClassName("f-by-b-name");
let filter_btnDelivery = document.getElementsByClassName("f-by-date");
let sort_btn = document.getElementsByClassName("s-items_order");
...
// Ham Menu
function pop() {
    document.getElementById("ham-navigation").style.height = "200px"
}
function close_nav() {
    document.getElementById("ham-navigation").style.height = "0px"
}
...
function expand_filter() {
    ...
    if (filter_menu == "brand") {
        document.getElementById("f-bb-name").style.display = 'flex'
    }
    else if (filter_menu == "Price") {
        document.getElementById("f-by-p").style.display = 'flex'
    }
    else if(filter_menu == 'Star Rating'){
        document.getElementById("f-b-rate").style.display = 'flex'
    }
    else {
        document.getElementById('f-b-date').style.display = 'flex'
    }
}
...
function expand_sort(){
    ...
    if((sort_menu == 'brand') || (sort_menu == 'price') || (sort_menu == 'rating')){
        document.getElementById("s-items").style.display = 'flex'
    }
}

// quantity counter for all
document.getElementById('qantity').innerText = filter_productByName.length

// logic for filter by brand name
for (i=0; i<filter_btnBrand.length; i++){
    filter_btnBrand[i].addEventListener('click', (e) =>{
        e.preventDefault()
        document.getElementById("f-bb-name").style.display = 'none'
        ...
        filter_productByName.forEach((brand_div)=>{
            if(brand_div.innerText.includes(filter_by_brand)){
                ...
                count+=1
                document.getElementById("qantity").innerText = count;
            }
            else{
...
function filter_price(a, b){
    document.getElementById("f-by-p").style.display = 'none'
...
function filter_rating(a){
    document.getElementById("f-b-rate").style.display = 'none'
...
for(i=0; i<filter_btnDelivery.length; i++){
    filter_btnDelivery[i].addEventListener('click', (e) =>{
        e.preventDefault()
        document.getElementById('f-b-date').style.display = 'none'
...
for(i=0; i<sort_btn.length; i++){
    sort_btn[i].addEventListener('click', (e) =>{
        document.getElementById("s-items").style.display = 'none'
        let sort_menu = document.getElementById("sort").value;
